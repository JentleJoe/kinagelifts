...
const Achievements = () => {
  ...
  const startCountingAnimation = () => {
    achievementsData.forEach((item) => {
      const finalValue = parseInt(item.value.replace(/[^0-9]/g, ''))
      ...
      const timer = setInterval(() => {
        ...
        setAnimatedValues(prev => ({
          ...prev,
          [item.id]: item.value.includes('+') ? 
            `${Math.floor(currentValue)}+` : 
            item.value.includes('Year') ? 
              `${Math.floor(currentValue)} Year` :
  ...
  return (
    <section ref={sectionRef} className="py-16 bg-[#F9F9F9]">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-10">
            {achievementsData.map((item) => (
              <div key={item.id} className="flex items-center space-x-4 group">
                ...
                <div>
                  <p className="text-2xl font-bold text-[#1B1B1B] transition-all duration-300">
                    {isVisible ? (animatedValues[item.id] || '0') : '0'}
                  </p>
